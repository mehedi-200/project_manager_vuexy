...
import type { Feature, Phase, ChecklistItem, Comment } from '@/types/index.ts'
...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'updated', feature: Feature): void
}>()
...
watch(
  ...
  (f) => {
    if (f) {
      checklist.value = f.checklist ? [...f.checklist] : []
        ...
        <div :class="['drawer', { dark: isDark }]">
          <div class="drawer-header">
            ...
            <div class="drawer-badges">
              <Badge type="status" :value="feature.status" />
              <Badge type="priority" :value="feature.priority" />
            </div>
          </div>
          ...
          <div class="drawer-body">
            ...
            <div class="info-grid">
              <div class="info-item">
                ...
                <span class="info-value">{{ phaseName(feature.phase_id) }}</span>
              </div>
              <div class="info-item">
              ...
              <div class="info-item">
                ...
                <span class="info-value">{{ feature.day_number ?? '—' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">Serial</span>
                <span class="info-value">{{ feature.serial_number ?? '—' }}</span>
              </div>
            </div>
            ...
            <div class="section">
              ...
              <div class="checklist-progress">
                <div class="progress-bar">
                  <div
                    class="progress-fill"
                    :style="{ width: checklist.length ? (checklist.filter(c => c.is_completed).length / checklist.length * 100) + '%' : '0%' }"
                  ></div>
                </div>
              </div>
              <div class="checklist-list">
                <div v-for="item in checklist" :key="item.id" class="checklist-item">
                  ...
                  <button class="icon-btn-sm" @click="handleDeleteChecklist(item)" title="Delete">🗑️</button>
                </div>
              </div>
              <div class="add-row">
                <input
                ...
                />
                <button class="btn-add" :disabled="addingChecklist" @click="handleAddChecklist">
                  {{ addingChecklist ? '...' : '+ Add' }}
                </button>
              </div>
            </div>
            ...
            <div class="section">
              ...
              <div class="comment-input-row">
                <textarea
                  ...
                  rows="2"
                ></textarea>
                <button class="btn-post" :disabled="postingComment" @click="handlePostComment">
...
.drawer-close {
  ...
  opacity: 1; padding: 6px 10px; border-radius: 8px; color: inherit; flex-shrink: 0;
  transition: all 0.2s;
}
...
.info-item {
  padding: 12px; border: 1px solid rgba(102,126,234,0.12);
...
.add-input {
  flex: 1; padding: 8px 12px; border: 1px solid rgba(102,126,234,0.25);
  border-radius: 8px; font-size: 13px; outline: none; background: inherit; color: inherit;
}
.add-input:focus { border-color: #667eea; }

.btn-add {
  padding: 8px 16px; background: rgba(102,126,234,0.1); border: 1px solid rgba(102,126,234,0.3);
  border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; color: #667eea; white-space: nowrap;
  transition: all 0.2s;
}
...
.comment-textarea {
  width: 100%; padding: 10px 14px; border: 1px solid rgba(102,126,234,0.25);
...
.icon-btn-sm {
  ...
  padding: 2px 4px; border-radius: 4px; opacity: 0.5; color: inherit;
}
.icon-btn-sm:hover { opacity: 1; }

/* Drawer transition - animate the panel only */
...
.drawer.dark .add-input,
.drawer.dark .comment-textarea { background: #252d40; border-color: rgba(255,255,255,0.12); color: #e2e8f0; }
.drawer.dark .add-input::placeholder,
.drawer.dark .comment-textarea::placeholder { color: rgba(255,255,255,0.3); }
